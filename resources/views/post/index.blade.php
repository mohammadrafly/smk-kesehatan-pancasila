                ...
                <table class="table align-items-center justify-content-center mb-0">
                  ...
                  @foreach ($posts as $p)
                    <tr>
                      ...
                      <td>
                        <p class="text-sm font-weight-bold mb-0">{{ ($p->category->category) }}</p>
                      </td>
                      <td>
                        <span class="badge badge-sm bg-gradient-secondary">{{ $p->status }}</span>
                      </td>
                      <td>
                        <span class="text-xs font-weight-bold">{{ $p->slug }}</span>
                      </td>
                      <td>
                          <a class="btn btn-link text-danger text-gradient px-3 mb-0" href="{{ route('post.delete', $p->id)}}"><i class="far fa-trash-alt"></i>Delete</a>
                          <a class="btn btn-link text-dark px-3 mb-0" href="{{ route('post.edit', $p->id)}}"><i class="fas fa-pencil-alt text-dark" aria-hidden="true"></i>Edit</a>
                      </td>
                    </tr>
                    @endforeach
                  </tbody>
